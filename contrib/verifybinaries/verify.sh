...
###   konjocoincore.org and konjocoin.org and compares them.
...
WORKINGDIR="/tmp/konjocoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://konjocoincore.org"
HOST2="https://konjocoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="konjocoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'konjocoin-',
...
if [ $? -ne 0 ]; then
   echo "konjocoin.org failed to provide signature file, but konjocoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "konjocoin.org and konjocoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Konjocoin Core binary release signing key installed?"
   fi
