...
git clone https://github.com/konjocoin-core/qa-assets ${DIR_FUZZ_IN}
...
env | grep -E '^(KONJOCOIN_CONFIG|CCACHE_|WINEDEBUG|LC_ALL|BOOST_TEST_RANDOM|CONFIG_SHELL|(ASAN|LSAN|TSAN|UBSAN)_OPTIONS)' | tee /tmp/env
if [[ $HOST = *-mingw32 ]]; then
  DOCKER_ADMIN="--cap-add SYS_ADMIN"
elif [[ $KONJOCOIN_CONFIG = *--with-sanitizers=*address* ]]; then # If ran with (ASan + LSan), Docker needs access to ptrace (https://github.com/google/sanitizers/issues/764)
...
if [ "$KONJOCOIN_SCRYPT" = 1 ]; then 
  travis_retry DOCKER_EXEC pip3 install konjocoin_scrypt
fi
