Konjocoin Core version 0.18.1 is now available from:

  <https://download.konjocoin.org/konjocoin-0.18.1/>
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/konjocoin-project/konjocoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@konjocoin.org) the Konjocoin Core
...
- A new short [document](https://github.com/konjocoin-project/konjocoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/konjocoin-project/konjocoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/konjocoin-project/konjocoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/konjocoin-project/konjocoin/blob/master/doc/konjocoin-conf.md)
  about the `konjocoin.conf` file describes how to use it to configure
  Konjocoin Core.

- A new document introduces Konjocoin Core's BIP174 [Partially-Signed
  Konjocoin Transactions
  (PSBT)](https://github.com/konjocoin-project/konjocoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/konjocoin-project/konjocoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Konjocoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Konjocoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/konjocoin-project/konjocoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/konjocoin-project/konjocoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Konjocoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Konjocoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/konjocoin-project/konjocoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/konjocoin-project/konjocoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart konjocoind with the `-deprecatedrpc=generate`
...
- A new `konjocoin-wallet` tool is now distributed alongside Konjocoin
...
This section describes planned changes to Konjocoin Core that may affect
other Konjocoin software and services.

- Since version 0.16.0, Konjocoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Konjocoin Core 0.20 (expected about
  a year after 0.18), Konjocoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Konjocoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Konjocoin Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any Konjocoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Konjocoin Core
...
- A sub-project of Konjocoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Konjocoin Core.  See their [project
  page](https://github.com/konjocoin-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Konjocoin Core's own implementation, although entropy
  gathered by Konjocoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Konjocoin Core a
...
- On macOS, Konjocoin Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `konjocoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libkonjocoin_server.a` (jnewbery)
...
- #13926 [Tools] konjocoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like konjocoind (MarcoFalke)
...
- #14521 qt, docs: Fix `konjocoin-qt -version` output formatting (hebasto)
...
- #11625 Add KonjocoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libkonjocoin_server` (laanwj)
...
- #15154 Configure: konjocoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/konjocoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Konjocoin Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libkonjocoin_server` (Mr-Leshiy)
...
- #15947 Install konjocoin-wallet manpage (domob1812)
- #15983 build with -fstack-reuse=none (MarcoFalke)

### Tests and QA
- #15405 appveyor: Clean cache when build configuration changes (Sjors)
- #13953 Fix deprecation in konjocoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into Konjocoin Core (Christewart)
...
- #12246 Bugfix: Only run konjocoin-tx tests when konjocoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_konjocoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_konjocoin` (MarcoFalke)
...
- #14186 konjocoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_konjocoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update konjocoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/konjocoin-conf.md (hebasto)
...
- #15354 Add missing `konjocoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct konjocoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support konjocoin folder on OSX (JimmyMow)
...
- [The Konjocoin Core Developers](https://github.com/konjocoin/konjocoin/tree/master/doc/release-notes)
- Adrian Gallagher
- aunyks
- coblee
- cryptonexii
