Konjocoin Core version 0.14.0 is now available from:

  <https://konjocoin.org/bin/konjocoin-core-0.14.0/>
...
Konjocoin Core has supported automatically pruning the blockchain since 0.11. Pruning
...
now ZMQ can be used on Windows. Please see [this document](https://github.com/konjocoin/konjocoin/blob/master/doc/zmq.md) for
...
The Alert System was [disabled and deprecated](https://konjocoin.org/en/alert/2016-11-01-alert-retirement) in Konjocoin Core 0.12.1 and removed in 0.13.0. 
...
- #8722 `bb843ad` konjocoin-cli: More detailed error reporting (laanwj)
...
- #8830 `ef0801b` test: Add option to run konjocoin-util-test.py manually (jnewbery)
- #8881 `e66cc1d` Add some verbose logging to konjocoin-util-test.py (jnewbery)
...
- #9023 `774db92` Add logging to konjocoin-util-test.py (jnewbery)
- #9065 `c9bdf9a` Merge `doc/unit-tests.md` into `src/test/README.md` (laanwj)
- #9069 `ed64bce` Clean up bctest.py and konjocoin-util-test.py (jnewbery)
...
- #9691 `fc67cd2` Init ECC context for `test_konjocoin_fuzzy` (gmaxwell)
...
- #9865 `289204f` Change konjocoin address in RPC help message (marijnfs)
...
- #8813 `fb24d7e` konjocoind: Daemonize using daemon(3) (laanwj)
...
- #8837 `2108911` Allow konjocoin-tx to parse partial transactions (jnewbery)
- #9204 `74ced54` Clarify CreateTransaction error messages (instagibbs)
- #9265 `31bcc66` konjocoin-cli: Make error message less confusing (laanwj)
...
- #8883 `d5d4ad8` Add all standard TXO types to konjocoin-tx (jnewbery)
