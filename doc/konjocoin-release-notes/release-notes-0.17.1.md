Konjocoin Core version 0.17.1 is now available from:

  <https://download.konjocoin.org/konjocoin-0.17.1/>
...
When konjocoin is configured with the `-deprecatedrpc=accounts` setting, specifying
...
- `-includeconf=<file>` can be used to include additional configuration files.
  Only works inside the `konjocoin.conf` file, not inside included files or from
  ...
  as konjocoin.conf will still include `relative.conf`.
...
Previously, wallets could only be loaded or created at startup, by specifying `-wallet` parameters on the command line or in the konjocoin.conf file. It is now possible to load, create and unload wallets dynamically at runtime:
...
prefixing the option with the network, such as:
    ...
    main.uacomment=konjocoin
    test.uacomment=konjocoin-testnet
...
be used in V0.17 by starting konjocoind with the '-deprecatedrpc=accounts'
...
BIP 174 Partially Signed Konjocoin Transactions support
-----------------------------------------------------

[BIP 174 PSBT](https://github.com/konjocoin/bips/blob/master/bip-0174.mediawiki) is an interchange format for Konjocoin transactions that are not fully signed
yet, together with relevant metadata to help entities work towards signing it.
It is intended to simplify workflows where multiple parties need to cooperate to
produce a transaction. Examples include hardware wallets, multisig setups, and
[CoinJoin](https://konjocointalk.org/?topic=279249) transactions.

For backend RPC convenience, the Konjocoin devs have supported to keep the acronym `PSBT`
instead of `PSLT` to make crosschain application support easier.

### Overall workflow

Overall, the construction of a fully signed Konjocoin transaction goes through the
following steps:
...
- An **Extractor** produces a valid Konjocoin transaction (in network format)
...
- The new RPC `getzmqnotifications` returns information about active ZMQ
  notifications.
- When konjocoin is not started with any `-wallet=<path>` options, the name of
  the default wallet returned by `getwalletinfo` and `listwallets` RPCs is
  now the empty string `""` instead of `"wallet.dat"`. If konjocoin is started
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart konjocoind with
...
- When running konjocoind with `-debug` but without `-daemon`, logging to stdout
...
meaning the `-txindex` flag can be toggled without a full reindex. If konjocoind
...
- #11962 `0a01843` add seed.konjocoin.sprovoost.nl to DNS seeds (Sjors)
...
- #12723 `310dc61` Qt5: Warning users about invalid-BIP21 URI konjocoin:// (krab)
...
- #13528 `b877c39` Move KonjocoinGUI initializers to class, fix initializer order warning (laanwj)
...
- #13486 `66e1a08` Move rpc/util.cpp from libkonjocoin-util to libkonjocoin-server (ken2812221)
...
- #13604 `75bea05` Add depends 32-bit arm support for konjocoin-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling konjocoin-qt (TheCharlatan)
...
- #14698 `ec71f06` Add konjocoin-tx.exe into Windows installer (ken2812221)
...
- #11771 `2dbc4a4` Change invalidtxrequest to use KonjocoinTestFramework (jnewbery)
...
- #12443 `07090c5` Move common args to konjocoin.conf (MarcoFalke)
...
- #11772 `0630974` Change invalidblockrequest to use KonjocoinTestFramework (jnewbery)
...
- #11773 `f0f9732` Change `feature_block.py` to use KonjocoinTestFramework (jnewbery)
- #12839 `40f4baf` Remove travis checkout depth (laanwj)
- #11817 `2a09a78` Change `feature_csv_activation.py` to use KonjocoinTestFramework (jnewbery)
...
- #12856 `27278df` Add Metaclass for KonjocoinTestFramework (WillAyd)
...
- #12882 `d96bdd7` Make `test_konjocoin` pass under ThreadSanitzer (clang). Fix lock-order-inversion (potential deadlock) (practicalswift)
...
- #13687 `9d26b69` travis: Check that ~/.konjocoin is never created (MarcoFalke)
...
- #13811 `660abc1` travis: Run `bench_konjocoin` once (MarcoFalke)
...
- #12102 `7fb8fb4` Apply hardening measures in konjocoind systemd service file (Flowdalic)
- #12652 `55f490a` konjocoin-cli: Provide a better error message when konjocoind is not running (practicalswift)
...
- #12769 `35eb9d6` Add systemd service to konjocoind in debian package (ghost)
...
- #13374 `56f6936` utils and libraries: checking for konjocoin address in translations (kaplanmaxe)
...
- #13603 `453ae5e` konjocoin-tx: Stricter check for valid integers (domob1812)
...
- #13725 `07ce278` Fix konjocoin-cli --version (Empact)
...
- #12877 `2b54155` Use konjocoind in Tor documentation (knoxcard)
...
- #13346 `60f0358` update konjocoin-dot-org links in release-process.md (fanquake)
...
- #13742 `d2186b3` Adjust konjocoincore.org links (MarcoFalke)
...
- #13905 `73a09b4` fixed konjocoin-cli -help output for help2man (hebasto)
