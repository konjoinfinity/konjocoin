Konjocoin Core version 0.14.2 is now available from:

  <https://download.konjocoin.org/konjocoin-0.14.2.0/>
...
Konjocoin Core now supports P2SH addresses beginning with M on mainnet and Q on testnet.
...
This only affects users that have explicitly enabled UPnP through the GUI setting or through the -upnp option, as since the last UPnP vulnerability (in Konjocoin Core 0.10.4) it has been disabled by default.
...
- http://testnet.konjocointools.com
- http://testnet.thrasher.io

Developers who require the new testnet blockchain paramaters can find them [here](https://github.com/konjocoin-project/konjocoin/blob/master/src/chainparams.cpp#L220).
...
Konjocoin Core has supported automatically pruning the blockchain since 0.13.2. Pruning
...
now ZMQ can be used on Windows. Please see [this document](https://github.com/konjocoin-project/konjocoin/blob/master/doc/zmq.md) for
...
When Konjocoin Core is out-of-sync on startup, a semi-transparent information
...
Commands sent over the JSON-RPC interface and through the `konjocoin-cli` binary
can now use named arguments. This follows the [JSON-RPC specification](http://www.jsonrpc.org/specification)
for passing parameters by-name with an object.

`konjocoin-cli` has been updated to support this by parsing `name=value` arguments
when the `-named` option is given.

Some examples:

    src/konjocoin-cli -named help command="help"
    src/konjocoin-cli -named getblockhash height=0
    src/konjocoin-cli -named getblock blockhash=000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f
    src/konjocoin-cli -named sendtoaddress address="(snip)" amount="1.0" subtractfeefromamount=true
...
The Alert System was disabled and deprecated in Konjocoin Core 0.10.4 and removed in 0.13.2. 
...
----------------------
 ...
 - A new RPC command `getmemoryinfo` has been added which will return information
   about the memory usage of Konjocoin Core. This was added in conjunction with
   optimizations to memory management. See [Pull #8753](https://github.com/konjocoin/konjocoin/pull/8753)
...
here, originally written for Konjocoin but can also be used for Konjocoin:
[reducing-konjocoind-memory-usage.md](https://gist.github.com/laanwj/efe29c7661ce9b6620a7).
